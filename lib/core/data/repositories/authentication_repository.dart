...
class AuthenticationRepository implements IAuthenticationRepository {
  ...
  Future<Either<AuthFailure, List<PlanModel>?>> getPlans() async {
   ...
   @override
  Future<Either<AuthFailure, List<PaymentModel>?>> getPayments() async {
    try {
      final isConnected = await networkInfo.isConnected;
      if (isConnected == false) return Left(AuthFailure.network());
  ...
  @override
  Future<Either<AuthFailure, CouponModel?>> checkCouponStatus(String code) async {
    try {
      final isConnected = await networkInfo.isConnected;
      if (isConnected == false) return Left(AuthFailure.network());
      final response = await remoteDataSource.checkCouponStatus(code);
      return Right(response);
    } on UnauthorizedExeption {
      return Left(AuthFailure.unauthorized());
    } on JwtExpiredExeption {
